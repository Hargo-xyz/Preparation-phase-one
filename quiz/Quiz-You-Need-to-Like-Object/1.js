...
function ganjilGenapAsianGames(date, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        let temp = data[i].plat.split(' ')
        let cek = temp[1]
        let lastIdx = 0;
        for (let j = 0; j < cek.length; j++) {
            if (j === cek.length - 1) {
                lastIdx = Number(cek[j])
            }
        }
        // console.log(date, cek, lastIdx, data[i].type);
        if (lastIdx % 2 != 0 && date % 2 == 0 && data[i].type == 'Mobil') {
            count++
        } else if (lastIdx % 2 == 0 && date % 2 != 0 && data[i].type == 'Mobil') {
            count++
        }
    }
    return count
